# Copied from:
# https://github.com/belyalov/tinyweb/blob/master/tinyweb/server.py
        ...
        try:
            ...
            if e.args[0] not in (errno.ECONNABORTED, errno.ECONNRESET, 32):
                try:
                    ...
                    print("exception line 480: ",e)
                    #log.exc(e, "")
        except HTTPException as e:
            try:
                await resp.error(e.code)
            except Exception as e:
                print("Exception lin 486:",e)
                #log.exc(e)
        except Exception as e:
            # Unhandled expection in user's method
            print("Unhandled expection in user's method:", req.path.decode())
    ...
    async def _tcp_server(self, host, port, backlog):
        ...
        try:
            while True:
                ...
                # https://github.com/micropython/micropython/blob/master/examples/network/http_server_ssl.py
